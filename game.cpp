...
Discentes / Matrículas:
	Francisco Bruno Duarte Castro - 1440480
	Gabriel Furtado Lins Melo - 1394225
	Hianuy Esperidião de Sousa Pinto - 1394198
*/
...
// x ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ x ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ x ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ x
void inicializar (void) { /* Inicializa parâmetros de rendering */
...
void gerenciarEventos() {
	...
	onda.run();
	caldo();
	if(hero.is_carrying_treasure()) tesouro.set_position(hero.get_x(), hero.get_y()+hero.get_radius(), hero.get_z());
	tesouro.update();
...
void caldo() {
	if(hero.check_lost(onda.get_x(), onda.get_z())) perder();
}
...
void gerenciarMovimentos(int v) {
	if(_up) hero.move_forward(psi);
	if(_down) hero.move_back(psi);
	if(_left) hero.move_left(psi);
	if(_right) hero.move_right(psi);
	if(_jump) hero.jump();
	if(_increase) hero.increase();
	if(_decrease) hero.decrease();
...
void mouseFunc(int button, int state, int x, int y){
	...
	if(full_screen) { // Ajuste das coordenadas dos botões na tela cheia.
		x -= 110;
		y -= 35;
		fs_x = 50;
		fs_y = -5;
	...
	if(lost) {
		if(x >= 450 && x <= 650 + fs_x) {
			if(y >= 350 + fs_y && y <= 400 + fs_y) reiniciar();
			else if(y >= 410 + fs_ex && y <= 460 + fs_ex) exit(0);
		}
	}
	if(!paused) {
		...
		if(button==3 && camera_distance >= 10.0 + hero.get_radius()) camera_distance -= 2.0;
		else if(button==4 && camera_distance <= 60.0) camera_distance += 2.0;
	}
	else {
		if(x >= 450 && x <= 650 + fs_x) {
			if(y >= 230 + fs_y && y <= 280 + fs_y) despausar(); // CONTINUAR
			else if(y >= 290 + fs_y && y <= 340 + fs_y) reiniciar();
			else if(y >= 350 + fs_y && y <= 400 + fs_y) tela_cheia();
			else if(y >= 410 + fs_ex && y <= 460 + fs_ex) exit(0);
		}
	}
}
...
void manageMouseRotation(int x, int y) {
	...
	if(theta<=-89.0) theta=-89.0;
	if(psi>=360.0) psi-=360.0;
