...
public:
	...
	void set_position(float _x, float _y, float _z) {x=_x; y=_y; z=_z;}
	void respawn();
	void draw();
	void update();
};
...
public:
	...
	void set_position(float x_, float y_, float z_) {x=x_;y=y_;z=z_;}
	...
	void move_forward(float psi) {speed_x+=move_x(psi); speed_z+= move_z(psi);}
	void move_back(float psi) {speed_x-=move_x(psi); speed_z-= move_z(psi);}
	void move_left(float psi) {speed_x+=move_z(psi); speed_z-= move_x(psi);}
	void move_right(float psi) {speed_x-=move_z(psi); speed_z+= move_x(psi);}
	void jump(); /*{speed_y+=speed_rate;}*/
	void increase() {if(radius<=7.0) radius += 0.1;}
	void decrease() {if(radius>=0.4) radius -= 0.1;}
	...
	void get_heavier();
	void get_lighter();
	void stop() {speed_x=0.0; speed_y=0.0; speed_z=0.0;}
};
...
bool Ball::check_lost(float wave_x, float wave_z) {
	...
	if(z >= z_wave_func-5 && z <= z_wave_func+10 && y <= 6.0) return 1;
	else return 0;
}
...
void Ball::gravity() {
	...
	if(y<radius) {
		...
		if(speed_y<=-0.1) force(direction, speed_y*0.9);
		y=radius;
	}
}
void Ball::force(float dir[], float norm) {
	if(dir[0]) speed_x = -1*dir[0]*norm;
	if(dir[1]) speed_y = -1*dir[1]*norm;
	if(dir[2]) speed_z = -1*dir[2]*norm;
}
void Ball::jump() {
	float dir[] = {0.0,1.0,0.0};
	if(y==radius) force(dir, -jump_force);
}
void Ball::draw() {
	glTranslatef(x, y, z);
	float Ka[] = {0.3,0.3,0.0};
...
Wave::Wave() {
	quad = gluNewQuadric();
	wave_x = -230.0; wave_y = 1.0; wave_z = 230.0;
	wave_speed = 0.5;
}
void Wave::restart() {
	wave_x = -230.0; wave_y = 1.0; wave_z = 230.0;
